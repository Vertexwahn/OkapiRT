...
# ---------- Data Gathering ----------
method_titles = ['Mitsuba Spectral', 'Mitsuba RGB']
method_filenames = ['cornell_box.path.integrator.spp1024.mitsuba.spectral.exr',
                    'cornell_box.path.integrator.spp1024.mitsuba.exr' ]
...
crops = [
    util.image.Cropbox(top=120, left=400, height=50, width=40, scale=5),
    util.image.Cropbox(top=581, left=295, height=50, width=40, scale=5)
]
crop_colors = [
    [255,110,0],
    [0,200,100]
...
if __name__ == "__main__":
    figuregen.figure(v_grids, width_cm=18., filename='spectral_vs_rgb.pdf')
    figuregen.figure(v_grids, width_cm=18., filename='spectral_vs_rgb.pptx')
    figuregen.figure(v_grids, width_cm=18., filename='spectral_vs_rgb.html')

    try:
        from figuregen.util import jupyter
        jupyter.convert('spectral_vs_rgb.pdf', 300)
    except:
